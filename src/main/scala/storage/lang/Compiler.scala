...
object Compiler extends RegexParsers with PackratParsers {
  ...
  /*private def enumVariableDefinition: Parser[EnumVariableDefinition] = {
    identifier ~ ENUM ~ LEFT_PARENTHESIS ~ rep1sep(enumElement, COMMA) ~ RIGHT_PARENTHESIS ~ comment ^^ {
      case path ~ _ ~ _ ~ enumElements ~ _ ~ comment =>
        EnumVariableDefinition(path, comment, enumElements)
    }
  }

  private def enumElement: Parser[EnumElementDefinition] = {
    identifier ~ comment ^^ {
      case path ~ comment => EnumElementDefinition(path, comment)
    }
  }*/
  ...
  //  private def switchStatement: Parser[Operation] = {
  //    prefix ~ SWITCH ~ rep(caseStatement) ~ opt(otherwiseCondition) ^^ {
  //      case (operation, comment) ~ _ ~ statements ~ otherwise =>
  //        ExclusiveGatewayOperation(operation, comment, statements ::: otherwise.map(_ :: Nil).getOrElse(Nil))
  //    }
  //  }
  //
  ...
  //        CaseBranch(commentOpt.getOrElse(""), expr, nextOperation)
  //    }
  //  }
  //
  ...
  //        OtherwiseBranch(commentOpt.getOrElse(""), nextOperation)
  //    }
  //  }
  //
  //  private def letStatement: Parser[Operation] = {
  //    prefix ~ LET ~ rep1sep(assignmentStatement, COMMA) ~ goto ^^ {
  //      case (operation, comment) ~ _ ~ statements ~ nextOperation =>
  //        ScriptTaskOperation(operation, comment, statements, nextOperation)
  //    }
  //  }
  //
  ...
  //          MailOperation(operation, comment, adresses, subject, messsage, nextOperation)
  //      }
  //  }
  ...
  private def assignmentStatement: Parser[Assignment] =
    (identifier <~ ASSIGNMENT) ~ expression ^^ { case path ~ expr => Assignment(path, expr) }
  ...
  private def scopePattern = "\\$?"

  private def identifier: Parser[String] = {
    s"$scopePattern$namePattern(\\.$namePattern)*".r ^^ { str => str }
  }
