...
import storage.Storage.Request
...
class StorageSystem(val storageSystem: ActorRef)(implicit val c: ExecutionContext, val t: Timeout) extends StorageSystemLike
...
object StorageSystem {
  ...
  case class GetNodeRef(nodeId: PersistenceId) extends Request
  ...
  case class NodeRefOpt(x: Option[ActorRef]) extends Response
